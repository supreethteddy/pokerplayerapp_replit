                    ...
                    {tables?.map((table) => (
                      ...
                      >
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                          ...
                          {(() => {
                            const existingRequest = seatRequests?.find(req => req.tableId === table.id);
                            const isInWaitlist = existingRequest !== undefined;
                            
                            return (
                              <Button 
                                onClick={() => !isInWaitlist && joinWaitListMutation.mutate(table.id)}
                                disabled={joinWaitListMutation.isPending || isInWaitlist}
                                className={`w-full sm:w-auto text-xs sm:text-sm py-2 sm:py-3 ${
                                  isInWaitlist 
                              ...
                              >
                                {joinWaitListMutation.isPending 
                                  ? 'Joining...' 
                                  : isInWaitlist 
                                    ? `In Waitlist (${existingRequest.status})`
                                    : 'Join Wait-List'}
                              </Button>
                            );
                          })()}
