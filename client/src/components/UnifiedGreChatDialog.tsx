...
const PushNotificationManager = () => {
  ...
  const loadChatHistory = async () => {
    ...
    try {
      console.log(`📋 [CHAT] Loading chat history for player ${playerId}...`);
      const response = await fetch(`/api/unified-chat/messages/${playerId}`);

      if (response.ok) {
        const chatHistory: ChatMessage[] = await response.json();
        console.log(`✅ [CHAT] Loaded ${chatHistory.length} messages:`, chatHistory);
        setMessages(chatHistory);
      } else {
        console.error('❌ [CHAT] Failed to load chat history:', response.statusText);
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!playerId || !playerName) {
      console.log('🔍 [CHAT DEBUG] Missing player data:', { playerId, playerName });
      return;
    }
    ...
    try {
      ...
      const playerChannel = pusherClient.subscribe(`player-${playerId}`);

      playerChannel.bind('new-gre-message', (data: PusherMessage) => {
        console.log('📨 [PUSHER] Received GRE message:', data);
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogTrigger asChild>
          <Button variant="outline">
            {unreadCount > 0 && (
              <Badge className="absolute -top-2 -right-2 rounded-full px-2 py-1 text-xs bg-red-500 text-white">
                {unreadCount}
              </Badge>
            )}
              ...
              <ScrollArea className="h-[400px] mb-4">
                <div className="flex flex-col space-y-4">
                  {messages.map((msg, index) => (
                    <div
                      key={msg.id || index}
                      className={`flex ${msg.sender === 'player' ? 'justify-end' : 'justify-start'} mb-4`}
                    >
                      <div className={`max-w-[80%] ${msg.sender === 'player'
                  ...
                  {isLoading ? (
                    <>
                      <svg className="animate-spin h-5 w-5 mr-2" viewBox="0 0 24 24">
                        <path fill="currentColor" d="M12 4V2m0 16v2m8-8h2M4 12H2M17.66 6.34l1.42-1.42M6.34 17.66l-1.42 1.42M17.66 17.66l1.42 1.42M6.34 6.34l-1.42-1.42" />
                      </svg>
                      Sending
                    </>
                  ) : (
